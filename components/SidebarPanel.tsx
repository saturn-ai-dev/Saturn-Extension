    ...
    return (
        <div className="flex flex-col h-full bg-zen-bg p-5">
            <div className="flex-1 relative group">
                ...
                <textarea className="relative w-full h-full bg-zen-surface border border-zen-border rounded-[16px] p-4 text-sm text-zen-text focus:outline-none focus:border-zen-accent focus:ring-1 focus:ring-zen-accent/50 resize-none font-mono leading-relaxed shadow-inner" placeholder="// Scratchpad initialized..." value={note} onChange={(e) => setNote(e.target.value)} />
            </div>
...
const CalculatorWidget = () => {
    ...
    const btnClass = "h-14 rounded-[12px] bg-zen-surface border border-zen-border/50 hover:bg-zen-surface/80 hover:border-zen-accent/30 hover:text-zen-text text-zen-muted font-semibold text-lg transition-all active:scale-95 shadow-sm";
    const opClass = "h-14 rounded-[12px] bg-zen-bg border border-zen-border text-zen-accent font-bold text-xl hover:bg-zen-accent hover:text-white transition-all shadow-sm";
    const sciClass = "h-14 rounded-[12px] bg-zen-surface/30 border border-zen-border/30 text-zen-muted text-xs font-bold hover:bg-zen-surface hover:text-zen-text transition-all";

    return (
        <div className="flex flex-col h-full bg-zen-bg p-6 pb-8">
            <div className="bg-zen-surface/50 rounded-[20px] p-5 mb-6 border border-zen-border/70 shadow-inner relative overflow-hidden flex flex-col justify-end min-h-[140px]">
                {history.length > 0 && (
...
const SidebarPanel: React.FC<SidebarPanelProps> = ({ isOpen, appId, onClose, sidebarWidth, position = 'left' }) => {
    ...
    return (
        <div
            className={`fixed top-4 bottom-4 ${widthClass} bg-zen-surface/95 backdrop-blur-2xl border border-zen-border/50 shadow-deep z-40 transform transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col overflow-hidden ${isLeft ? 'rounded-[0_24px_24px_0]' : 'rounded-[24px_0_0_24px]'} ${isOpen ? 'translate-x-0' : (isLeft ? '-translate-x-full' : 'translate-x-full')}`}
            style={{
                left: isLeft ? `${sidebarWidth + 14}px` : 'auto',
                right: isLeft ? 'auto' : `${sidebarWidth + 14}px`
            }}
        >
            <div className="h-14 border-b border-zen-border/50 flex items-center justify-between px-5 bg-zen-bg/50 flex-shrink-0 select-none">
                <span className="font-fraunces font-bold text-sm text-zen-text flex items-center gap-2"><GripVertical className="w-4 h-4 text-zen-muted/60" />{title}</span>
