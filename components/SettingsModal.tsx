...
const SettingsModal: React.FC<SettingsModalProps> = ({ 
  ...
  onAddCustomShortcut, onDeleteCustomShortcut
}) => {
                        ...
                        {isCreatingExt ? (
                             <div className="p-6 bg-zen-surface/50 rounded-2xl border border-zen-border space-y-4">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <input className="flex-1 bg-zen-bg border border-zen-border rounded-xl px-4 py-3 focus:border-zen-accent outline-none text-zen-text" placeholder="Extension Name" value={newExtName} onChange={e => setNewExtName(e.target.value)} autoFocus />
                                </div>
                                ...
                                <button onClick={handleCreateExtensionSubmit} disabled={!newExtName || !newExtInstruct} className="w-full bg-zen-text text-zen-bg font-bold py-3 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all">Install Extension</button>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <button onClick={() => setIsCreatingExt(true)} className="w-full py-4 border-2 border-dashed border-zen-border hover:border-zen-accent hover:bg-zen-surface rounded-2xl flex flex-col items-center justify-center gap-2 text-zen-muted hover:text-zen-text transition-all group">
                                    ...
                                    <span className="font-bold text-sm">Create / Import Extension</span>
                                </button>
                    ...
                    <div className="max-w-3xl">
                         {downloads.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {downloads.map(item => (
                                    <div key={item.id} className="flex items-center gap-4 p-4 bg-zen-surface/40 border border-zen-border/50 rounded-2xl">
                                        <div className="p-3 rounded-xl bg-zen-bg border border-zen-border/50 shadow-sm">
                                            {getDownloadIcon(item.type)}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-sm font-bold text-zen-text truncate">{item.filename}</div>
                                            <div className="text-xs text-zen-muted">{new Date(item.timestamp).toLocaleString()}</div>
                                        </div>
                                        {item.uri && (
                                             <a href={item.uri} download={item.filename} className="px-3 py-1.5 rounded-lg bg-zen-bg border border-zen-border text-xs font-bold hover:bg-zen-accent hover:text-white hover:border-transparent transition-colors">
                                                 Download
                                             </a>
                                        )}
                                    </div>
