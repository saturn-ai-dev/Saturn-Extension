...
const MELODIES = [
    ...
    },
    {
        ...
        bpm: 64,
        notes: [
            [293.66, 1.5], [349.23, 1], [392.00, 1], [440.00, 2],
            [392.00, 1], [349.23, 1], [293.66, 2],
            [246.94, 1], [293.66, 1], [329.63, 1], [392.00, 2],
            [349.23, 1], [293.66, 1], [220.00, 3],
        ]
    },
    {
        name: 'Orbit',
        ...
        bpm: 88,
        notes: [
            [261.63, 0.5], [329.63, 0.5], [392.00, 1], [329.63, 0.5],
            [293.66, 0.5], [261.63, 1], [196.00, 1],
            [220.00, 0.5], [261.63, 0.5], [329.63, 1], [293.66, 0.5],
            [261.63, 0.5], [220.00, 1], [196.00, 2],
        ]
    },
    {
        name: 'Aurora',
        ...
        bpm: 70,
        notes: [
            [174.61, 1], [220.00, 1], [261.63, 2], [329.63, 2],
            [293.66, 1], [261.63, 1], [220.00, 2],
            [196.00, 1], [246.94, 1], [293.66, 2], [349.23, 2],
    ...
    },
    {
        ...
        bpm: 58,
        notes: [
            [155.56, 2], [196.00, 1], [220.00, 1], [261.63, 2],
            [220.00, 1], [196.00, 1], [174.61, 3],
            [164.81, 2], [207.65, 1], [246.94, 1], [293.66, 2],
            [246.94, 1], [220.00, 1], [196.00, 3],
        ]
    },
    {
        name: 'Tides',
        ...
        bpm: 76,
        notes: [
            [196.00, 1], [246.94, 0.5], [293.66, 0.5], [329.63, 1],
            [293.66, 1], [246.94, 1], [220.00, 2],
            [196.00, 1], [220.00, 0.5], [261.63, 0.5], [293.66, 1],
            [261.63, 1], [220.00, 1], [196.00, 2],
        ]
    },
    {
        name: 'Signal',
        ...
        notes: [
            ...
            [440.00, 0.5], [392.00, 0.5], [329.63, 1],
            [293.66, 0.5], [349.23, 0.5], [392.00, 0.5], [440.00, 0.5],
            [392.00, 0.5], [349.23, 0.5], [293.66, 1.5],
        ]
    },
    {
        name: 'Afterglow',
        ...
        bpm: 62,
        notes: [
            [220.00, 2], [261.63, 1], [293.66, 1], [349.23, 2],
            [329.63, 1], [293.66, 1], [261.63, 2],
            [196.00, 2], [220.00, 1], [261.63, 1], [329.63, 2],
            [293.66, 1], [261.63, 1], [220.00, 2],
        ]
    },
];
...
class AmbientSynth {
    ...
    setVolume(v: number) {
        ...
        this.masterGain.gain.linearRampToValueAtTime(v, this.ctx.currentTime + 0.18);
    }
}
...
const AmbientPlayer: React.FC<AmbientPlayerProps> = ({ defaultOpen = false }) => {
    ...
    const stop = useCallback((suspend = true) => {
    ...
    const scheduleNext = useCallback(async (idx: number, melody: typeof MELODIES[0]) => {
        if (!playingRef.current) return;
        const { ctx, synth } = await ensureCtx();
        ...
        synth.playNote(freq, ctx.currentTime, durationSec, Math.max(0.12, volume * 0.85));
        setCurrentNote(idx);

        const nextIdx = (idx + 1) % melody.notes.length;
        timeoutRef.current = setTimeout(() => {
            scheduleNext(nextIdx, melody);
        }, durationSec * 1000 * 0.92);

        noteIdxRef.current = nextIdx;
    }, [ensureCtx, volume]);

    const start = useCallback(async (fromIdx = noteIdxRef.current) => {
        const melody = MELODIES[melodyIdx];
        playingRef.current = true;
        setIsPlaying(true);
        const { synth } = await ensureCtx();
        synth.setVolume(volume);
        noteIdxRef.current = fromIdx;
        scheduleNext(fromIdx, melody);
    }, [ensureCtx, melodyIdx, scheduleNext, volume]);
    ...
    useEffect(() => {
        if (isPlaying) {
            ...
            scheduleNext(0, MELODIES[melodyIdx]);
        }
    }, [melodyIdx, isPlaying, scheduleNext]);
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[150] flex flex-col items-end gap-2">
            {isExpanded && (
                <div className="glass-elevated w-[20rem] rounded-[24px] overflow-hidden animate-dropdown-open">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-zen-border/30">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                                <span className="text-sm font-semibold text-zen-text">Ambient</span>
                                <span className="text-[11px] text-zen-muted/70">Built-in loops</span>
                            </div>
                            {isPlaying && (
                                <div className="flex items-end gap-[2px] h-3">
                                    {[0, 1, 2].map(i => (
                                        <div
                                            key={i}
                                            className="w-[3px] bg-zen-accent rounded-full animate-bounce"
                    ...
                    <div className="px-4 pt-4 pb-2">
                        <div className="flex items-center justify-between mb-2">
                            <div className="text-[11px] font-medium text-zen-muted/70">Melody</div>
                            <div className="text-[10px] text-zen-muted/60">{MELODIES.length} options</div>
                        </div>
                        <div className="grid grid-cols-2 gap-1.5">
                            {MELODIES.map((m, i) => (
                                <button
                                    ...
                                    className={`text-left px-3 py-2.5 rounded-[14px] text-xs transition-all ${i === melodyIdx ? 'bg-zen-accent/15 text-zen-text border border-zen-accent/40 shadow-[0_18px_35px_-28px_rgba(var(--accent-color-rgb),0.75)]' : 'bg-zen-bg/60 text-zen-muted hover:text-zen-text border border-zen-border/40 hover:border-zen-border'}`}
                                >
                                    <div className="font-semibold">{m.name}</div>
                                    <div className="text-[10px] text-zen-muted/70 mt-0.5">{m.mood}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="px-4 pb-3">
                        <div className="flex items-center justify-between mb-2 text-[10px] text-zen-muted/65">
                            <span>{melody.mood}</span>
                    ...
                    <div className="px-4 pb-4">
                        <button
                            onClick={toggle}
                            className={`w-full py-3 rounded-[14px] text-sm font-semibold transition-all duration-200 ${isPlaying ? 'bg-zen-accent/10 text-zen-accent border border-zen-accent/40 hover:bg-zen-accent/20' : 'bg-zen-accent text-white hover:opacity-90 hover:-translate-y-0.5'}`}
                            style={{ boxShadow: isPlaying ? '0 0 20px var(--accent-glow)' : undefined }}
                        >
                            {isPlaying ? `Stop ${melody.name}` : `Play ${melody.name}`}
                        </button>
                    </div>
                </div>
            )}
            ...
            <button
                ...
                className={`glass-elevated flex items-center gap-2 px-4 py-2.5 rounded-full border transition-all duration-300 select-none ${isPlaying ? 'border-zen-accent/50 text-zen-accent' : 'border-zen-border/40 text-zen-muted hover:text-zen-text hover:border-zen-border'}`}
                style={{ boxShadow: isPlaying ? '0 4px 24px var(--accent-glow)' : '0 2px 12px rgba(0,0,0,0.15)' }}
            >
                {isPlaying ? (
