...
const RocketFuelGauge: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center gap-2 select-none">
            {/* Spinning Saturn instead of Rocket */}
            <div className="relative">
                <div className={`relative z-10 ${isEmpty ? 'opacity-40' : ''}`}>
                    <svg
                        viewBox="0 0 100 100"
                        className={`w-8 h-8 drop-shadow-lg transition-colors duration-500 ${isEmpty ? '' : 'animate-spin'}`}
                        style={{
                            animationDuration: `${1 * Math.pow(1 / 0.95, totalMax - totalRemaining)}s`,
                            color: isEmpty ? '#52525b' : isCritical ? '#ef4444' : isLow ? '#f59e0b' : 'var(--accent-color)'
                        }}
                    >
